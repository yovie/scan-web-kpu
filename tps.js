                    ...
                    async.waterfall([
                        ...
                        (cbb) => {
                            async.forEachOf(Area, (prov, num, cbbb) => {
                                ...
                                that.doGet(uriBaseArea + vcode + '.json', { code: prov.id }, (opt, kabkot) => {
                        ...
                        (cbb) => {
                            async.forEachOf(Area, (prov, num, cbbb) => {
                                async.forEachOf(prov.children, (kab, nu, cbbbb) => {
                                    ...
                                    this.doGet(uriBaseArea + prov.id + addpath + '/' + kab.id + '.json', { code: kab.id, provcode: prov.id, addpath: addpath }, (opt, kecamatan) => {
                            ...
                            async.forEachOf(Area, (prov, num, cbbb) => {
                                async.forEachOf(prov.children, (kab, nu, cbbbb) => {
                                    async.forEachOf(kab.children, (kec, n, cbbbbb) => {
                                        ...
                                        const numcode1 = numcode + 1
                                        let addpath = Number(prov.id) < 0 ? '/' + numcode0 + '/' + numcode1 : ''
                                        this.doGet(uriBaseArea + prov.id + addpath + '/' + kec.id + '.json', { code: kec.id, provcode: prov.id, addpath: addpath }, (opt, desa) => {
                                ...
                                async.forEachOf(prov.children, (kab, nu, cbbbb) => {
                                    async.forEachOf(kab.children, (kec, n, cbbbbb) => {
                                        async.forEachOf(kec.children, (desa, j, cbbbbbb) => {
                                            ...
                                            const numcode1 = numcode + 1
                                            let addpath = Number(prov.id) < 0 ? '/' + numcode0 + '/' + numcode1 : ''
                                            this.doGet(uriBaseArea + prov.id + addpath + '/' + desa.id + '.json', { code: desa.id, provcode: prov.id, addpath: addpath }, (opt, tps) => {
